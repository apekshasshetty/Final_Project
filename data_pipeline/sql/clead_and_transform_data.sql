-- sql/clean_and_transform_data.sql
...
BEGIN
  ...
  SELECT
    ...
    Product Id,
    Product Price,
    IFNULL(Product Rating, 0) AS Product Rating,
    IFNULL(Product Reviews Count, 0) AS Product Reviews Count,
    IFNULL(Product Category, '') AS Product Category,
    IFNULL(Product Brand, '') AS Product Brand,
    Product Name,
    Product Image Url,
    IFNULL(Product Description, '') AS Product Description,
    Product Tags
  FROM
    walmart_data.data;
